...
def writeheader(): 
  ...
  #f.write("#define PERCENT_ACCELERATABLE 200 // Each is 0.1% increase. 100 = 10%"+ "\n")
...
def write_accl_loop(f,dependency_before,dependency_after,iteration):
  ...
  write_line(" accl_accum" + str(iteration) + " = accl_accum" + str(iteration) + " * accl_accum" + str(iteration) + "; // our accelerator invocation")
...
def gen_dynamic_loop():
  ...
  for x in range(0,invocation_freq):
    ...
    while(read_val != 'n'): # try to get a normal loop to replace
      rand_int = randint(0,single_loop-1)
